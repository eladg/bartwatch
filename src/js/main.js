  ...
  for (var destIdx in trains) {
    for (var stationIdx in trains[destIdx]) {
      ...
      if (prev && prev.etaMins < train.etaMins) {
        ...
        train.etaMins > 10 &&
        train.prevStation != stationIdx &&
        next &&
        next.etaMins < train.etaMins
      ) {
        debug +=
          "<br> dir? " +
          train.forStation +
...
function drawLiveTrains(trains) {
  var renewTrains = [];
  var tselected = selectedTrain.split(".");
  var selectedTrainDest = tselected[0];
  var selectedTrainLine = tselected[1];
  var debug = "";
  for (var destIdx in trains) {
    for (var stationIdx in trains[destIdx]) {
      var train = trains[destIdx][stationIdx];
      if (
        (train.valid && selectedTrain === "all") ||
        (train.valid &&
          train.destStation == selectedTrainDest &&
          train.color == selectedTrainLine)
      ) {
        ...
        if (!exisiting) {
          ...
          position.lat != exisiting.position.lat ||
          position.lng != exisiting.position.lng
        ) {
          exisiting.train = train;
          exisiting.adjCount = 30;
          exisiting.adjLat =
            (position.lat - exisiting.position.lat) / exisiting.adjCount;
          exisiting.adjLng =
            (position.lng - exisiting.position.lng) / exisiting.adjCount;
          setTrainPopup(
            exisiting.marker,
            train,
            train.sta,
...
function extractPreviousLiveTrain(train, trains) {
  for (var i in trains) {
    var check = trains[i];
    if (
      train.destStation == check.train.destStation &&
...
function getRouteInfo(color, curr, dest) {
  ...
  var nextIdx = Math.min(
    Math.max(currIdx + (dirUp ? 1 : -1), 0),
    route.length - 1
  );
  var prevIdx = Math.min(
    Math.max(currIdx - (dirUp ? 1 : -1), 0),
    route.length - 1
  );
  return {
...
function setTrainPopup(
  ...
  if (estimate.minutes == "Leaving") {
    markerText +=
      "<br>Leaving Station: <b>" + stations[station.abbr].name + "</b>";
  } else {
    markerText +=
      "<br>Next Station: <b>" +
      stations[station.abbr].name +
      "</b> in " +
      estimate.minutes +
      " min" +
      debug(" (total " + threshold + ")");
  }
  ...
  if (estimate.delay > 0) {
    markerText +=
      "<br>Delayed: <b>" + secondsToMins(estimate.delay) + "</b> mins.";
  }
